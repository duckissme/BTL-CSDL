...
public class TimekeepingDAO {
    ...
    public Map<String, Object> saveTimekeeping(String employeeId, String date,
    ...
    private double calculateOvertime(String checkIn, String checkOut, String workStatus) {
        if (!"Đi làm".equalsIgnoreCase(workStatus)) {
            return 0;
        }
    ...
    public void generateDailyTimekeeping(String date) {
        ...
        List<String> employeeIds = jdbc.queryForList(sqlEmployees, String.class);
        ...
        for (String empId : employeeIds) {
            ...
            if (count == null || count == 0) {
                // Tạo timekeeping_id: YYYYMMDD_employeeId
                String timekeepingId = date.replace("-", "") + "_" + empId;

                String insertSql = """
                INSERT INTO timekeeping(timekeeping_id, employee_id, work_date, work_status, overtime)
                VALUES(?,?,?,?,?)
            """;
                jdbc.update(insertSql, timekeepingId, empId, date, "Đi làm", 0);
            }
        }
    }
}
